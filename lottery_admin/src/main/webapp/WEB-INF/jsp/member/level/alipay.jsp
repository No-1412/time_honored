                            ...
                            <div class="col-sm-12">
                                <form role="form" class="form-horizontal" id="form" data-url="system/memberLevel/alipaySubmit">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-5">
                                            <select class="form-control" name="alipay">
                                        	<option value="" label="">
                                   			<c:forEach var="alipay" items="${data.expand.alipayList}">
                                   			<c:if test="${alipay.pay_type==2}">
                                   			<option value="${alipay.id}" label="${alipay.name}(${alipay.code}) -- 扫码" ${alipay.checked==1?"selected":""}>
                                   			</c:if>
                                   			<c:if test="${alipay.pay_type==3}">
                                   			<option value="${alipay.id}" label="${alipay.name} -- 加好友" ${alipay.checked==1?"selected":""}>
                                   			</c:if>
                                   			<c:if test="${alipay.pay_type==5}">
                                   			<option value="${alipay.id}" label="${alipay.code}(${alipay.name}) -- ${alipay.bank}" ${alipay.checked==1?"selected":""}>
                                   			</c:if>
                                            </c:forEach>
                                            </select>
                                   		</div>
