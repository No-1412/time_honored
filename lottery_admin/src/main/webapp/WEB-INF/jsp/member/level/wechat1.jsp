                                ...
                                <form role="form" class="form-horizontal" id="form" data-url="system/memberLevel/wechatSubmit">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-5" style="margin-top: 5px;">
                                   			<c:forEach var="third" items="${data.expand.wechatList}">
                                   			<c:if test="${third.pay_type==2}">
                                   			<input type="checkbox" name="wechat" value="${third.id}" ${third.checked==1?"checked":""}>${third.name}(${third.code}) -- 扫码<br>
                                   			</c:if>
                                   			<c:if test="${third.pay_type==3}">
                                   			<input type="checkbox" name="wechat" value="${third.id}" ${third.checked==1?"checked":""}>${third.name} -- 加好友<br>
                                   			</c:if>
                                   			<c:if test="${third.pay_type==5}">
                                   			<input type="checkbox" name="wechat" value="${third.id}" ${third.checked==1?"checked":""}>${third.code}(${third.name}) -- ${third.bank}<br>
                                   			</c:if>
                                            </c:forEach>
                                   		</div>
                                        <div class="col-sm-5 form-control-static"></div>
