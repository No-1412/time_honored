...
<c:if test="${data != null && data.items != null}">
	<c:forEach var="access" items="${data.items}">
	    <tr>
	        <td>${access.lhcYear}</td>
	        <td>${access.lhcAnimals}</td>
         	<td><fmt:formatDate value="${access.addTime}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
	        <td>
	         	<a class="btn btn-sm btn-info edit_link" data-id="${access.id}" data-width="400px" data-height="420px" data-url="system/zodiac/edit" href="">编辑</a>
	         	<a class="btn btn-sm btn-danger cancel_link" data-id="${access.id}" data-url="system/zodiac/delete" href="">删除</a>
	        </td>
	    </tr>
    </c:forEach>
</c:if>
