...
@Controller
public class RechargeAlipayController extends AdminBaseController {
	@Autowired
	private RechargeTypeService rechargeTypeService;
	
	@RequestMapping(value="/system/alipay/index", method=RequestMethod.GET)
	public ModelAndView toIndex() throws Exception {
		return createSuccessModelAndView("recharge/alipay/table", null);
	}
	
	@RequestMapping(value="/system/alipay/getList", method=RequestMethod.GET)
	public ModelAndView getList(Integer page) throws Exception {
		List<Where> whereList = new ArrayList<Where>();
		whereList.add(Where.eq(RechargeWechatAlipayDomain.TYPE, 2));
		...
		return createSuccessModelAndView("recharge/alipay/page", resultPage);
	}
	
	@RequestMapping(value="/system/alipay/edit", method=RequestMethod.GET)
	public ModelAndView toEdit(Long id) throws Exception {
		...
		return createSuccessModelAndView("recharge/alipay/edit", rechargeBank);
	}
	...
	@ResponseBody
	@RequestMapping(value="/system/alipay/editSubmit", method=RequestMethod.POST)
	public CustomResponse editSubmit(RechargeWechatAlipay rechargeWechatAlipay) throws Exception {
		...
		if(isScan) {
			if(StringUtils.isBlank(rechargeWechatAlipay.getCode())) return createErrorJsonResponse("支付宝账号不能为空");
		...
		if(rechargeWechatAlipay.getId() == null) {
			rechargeWechatAlipay.setType(2);
		...
		rechargeTypeService.update(rechargeWechatAlipay, rechargeWechatAlipay.getId() == null, 2);
		
		return createSuccessJsonResponse(null);
	}
	
	@ResponseBody
	@RequestMapping(value="/system/alipay/delete", method=RequestMethod.POST)
	public CustomResponse delele(Long id) throws Exception {
		rechargeTypeService.delete(RechargeWechatAlipay.class, id, 2);
